<template>
	<view>
		<cu-custom bgColor="xg-nav" :isBack="true">
			...
			<block slot="content">生产计划单</block>
			<block slot="right">
		...
		<view class="shadow xg-search-sort bg-white">
			<view class="flex">
				<view class="flex-sub padding-sm text-center text-lg" :class="status==1?'bg-gblue':''" @tap="changeStatus(1)">待生产</view>
				<view class="flex-sub padding-sm text-center text-lg" :class="status==2?'bg-gblue':''" @tap="changeStatus(2)">生产中</view>
				<view class="flex-sub padding-sm text-center text-lg" :class="status==3?'bg-gblue':''" @tap="changeStatus(3)">已完成</view>
			</view>
		</view>
			...
			<view class="cu-dialog">
				...
				<view class="text-left">
					<form>
						...
						<view class="cu-form-group" @click="showSinglePicker(77)">
							<view class="title">排序</view>
							<input disabled="true" placeholder="请选择" :value="params['sort_name']"></input>
							<text class="cuIcon-right"></text>
						</view>
					</form>
				</view>
					...
					<view class="flex">
						<view class="flex-sub cf">
							<view class="fr">
								...
								<button v-if="item.status==1" class="cu-btn sm line-green margin-xs" @tap="confirmProduce(item.id)">确认生产</button>
								<button v-if="item.status!=3" class="cu-btn sm line-blue margin-xs" @tap="goToEdit(item.id)">修改</button>
								<button v-if="item.cut_status==1" class="cu-btn sm line-purple margin-xs" @tap="confirmCut(item.id)">开始裁剪</button>
								<button v-if="item.status!=3" class="cu-btn sm line-red margin-xs" @tap="ggDelete(item.id)">删除</button>
							</view>
						</view>
					</view>
