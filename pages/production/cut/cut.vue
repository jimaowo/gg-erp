<template>
	<view>
		<cu-custom bgColor="xg-nav" :isBack="true">
			...
			<block slot="content">更新裁剪</block>
			<block slot="right">
				<view class="flex padding-sm margin-xs justify-end" @tap="saveCut">
		...
		<form>
			...
			<view v-for="(detail,index) in params.plan_detail" :key="index" class="flex text-center bg-white">
				...
				<view class="flex-sub padding-sm solid">
					<input class="solids" type="text" v-model="detail['true_count']" />
				</view>
			</view>
			<view class="flex text-center bg-white">
				<view class="flex-sub padding-sm solid">合计</view>
				<view class="flex-sub padding-sm solid">{{getSumCount()}}</view>
			</view>
			
			<view class="padding-sm">裁床信息</view>
			<xg-input title="每层件数" v-model="params['cut_per_piece']"></xg-input>
			<xg-input title="备注" v-model="params['cut_remarks']"></xg-input>
