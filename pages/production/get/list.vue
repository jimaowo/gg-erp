<template>
	<view>
		<cu-custom bgColor="xg-nav" :isBack="true">
			...
			<block slot="content">外协收货单</block>
		</cu-custom>
		<view class="shadow xg-search-sort bg-white">
			...
			<uni-popup :show="showPop" position="middle" mode="fixed" @hidePopup="togglePopup">
				<view class="text-center">
					...
					<view class="flex padding-sm solid-bottom text-xl" @tap="changeSort('association_id asc')">按外协</view>
				</view>
			</uni-popup>
		</view>
		...
		<view class="cu-modal" :class="modalName=='DialogModal1'?'show':''">
			<view class="cu-dialog">
				...
				<view class="text-left">
					<form>
						<xg-input title="收货单号" v-model="params['code']"></xg-input>
						<xg-input title="发货单号" v-model="params['send_code']"></xg-input>
						...
						<view class="flex-treble">
							<view class="flex solid-bottom">
								<view class="flex-sub cf padding-left-sm">
									...
									<view class="fr">
										<text class="text-gblue">{{item.association_name}}</text>
									</view>
								</view>
							</view>
							<view class="flex margin-left-sm padding-top-sm">
								<view class="flex-sub">
									发货：<text class="text-red">{{item.send.send_count}}</text>
								</view>
								<view class="flex-sub text-right margin-right-sm">
									工价：<text class="text-red">{{item.send.wages}}</text>
								</view>
							</view>
							<view class="flex solid-bottom margin-left-sm">
								<view class="flex-sub">
									收货:<text class="text-red">{{item.send.receive_count}}</text>
								</view>
								<view class="flex-sub text-right margin-right-sm">
									{{item.create_time}}
								</view>
