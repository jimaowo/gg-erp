<template>
	<view>
		<cu-custom bgColor="xg-nav" :isBack="true">
			...
			<block slot="content">生产计划明细</block>
		</cu-custom>

		<form v-if="params.design!=undefined">
			...
			<view class="cu-form-group">
				...
				<input disabled="true" :value="params['batch']"></input>
			</view>
			
			<view class="padding-sm">计划数量</view>
			...
			<view v-for="(detail,index) in params.plan_detail" :key="index" class="flex text-center bg-white">
				...
				<view class="flex-twice padding-sm solid">{{detail.plan_count}}</view>
			</view>
			<view class="flex text-center bg-white">
				...
				<view class="flex-twice padding-sm solid">{{params.plan_count}}</view>
			</view>
			...
			<view v-show="infoTab==1" v-if="params.design">
				...
				<view class="cu-form-group">
					...
					<input v-if="params.type==1" disabled="true" value="本厂"></input>
					<input v-if="params.type==2" disabled="true" value="外协"></input>
					<input v-if="params.type==3" disabled="true" value="本厂+外协"></input>
				</view>
				<view class="cu-form-group">
					<view class="title">生产单位</view>
					<input disabled="true" :value="params['produce_company_name']"></input>
				</view>
			</view>
			...
			getEditOne() {
				api.get({
					...
					success: res => {
						...
						this.picList = this.params.plan_pic
					}
				});
			},
			getPickerData() {
