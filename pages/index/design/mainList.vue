<template>
	<view>
		<cu-custom bgColor="xg-nav" :isBack="true">
			...
			<block slot="content">主料</block>
			<block slot="right">
				<view class="flex padding-sm margin-xs justify-end" @tap="goToAdd">
					<text class="margin-xs text-xl">添加主布</text>
				</view>
			</block>
		</cu-custom>
				...
				<view class="text-left">
					<form>
						...
						<view class="grid col-2">
							...
							<xg-input title="货号" required=true v-model="params['item_no']"></xg-input>
							<xg-input title="色码" required=true v-model="params['color_no']"></xg-input>
							<xg-input title="预裁数" v-model="params['pre_cut']"></xg-input>
		...
		<view>
			<scroll-view scroll-x="true">
				...
				<view class="gg-scroll-view text-white">
					...
					<view class="gg-scroll-th">颜色</view>
					<view class="gg-scroll-th">单位</view>
					<view class="gg-scroll-th">预裁</view>
				...
				<view class="gg-scroll-view text-black" v-for="(item,index) in listData" :key="index">
					<view class="gg-scroll-td td-a">
						...
						<button v-if="item.is_match_color==1" class="cu-btn sm line-purple margin-left-xs" @tap="goToMatch(item.id)">拼色</button>
						<uni-badge v-if="item.is_match_color==2" class="margin-left-xl" text="拼" type="warning" size="small" />
						<!-- <view v-if="item.is_match_color==2" class='cu-tag round sm text-white bg-yellow'>拼</view> -->
					</view>
					...
					<view class="gg-scroll-td">{{item.color_name}}</view>
					<view class="gg-scroll-td">{{item.unit_name}}</view>
					<view class="gg-scroll-td">{{item.pre_cut || '-'}}</view>
