<template>
	<view>
		<cu-custom v-if="showTop" bgColor="xg-nav" :isBack="true">
			...
			<block slot="content">新增{{pageTitle}}</block>
			<block slot="right">
		...
		<form>
			...
			<view class="cu-form-group" @click="showSearchChoose(2)">
				<view class="title">款号<text class="text-red">*</text></view>
				<input disabled="true" placeholder="请选择" :value="params['name']"></input>
			...
			<view class="cu-form-group">
				<view class="title">{{pageTitle}}日期</view>
				<picker mode="date" :value="params['date']" :start="startDate" :end="endDate" id="date"
			...
			<view class="padding-sm">{{pageTitle}}明细</view>
			<view class="flex bg-white text-center">
				...
				<view class="flex-sub solid padding-sm text-bold">
					<input class="solids" type="text" placeholder="数量" @input="setAllCount" />
				</view>
			</view>
			<view v-for="(item, index) in detailList" :key="index" class="flex bg-white text-center">
				<view class="flex-sub solid padding-sm">{{item.color_name}}</view>
				<view class="flex-sub solid padding-sm">{{item.size_name}}</view>
				<view class="flex-sub solid padding-sm">
					<input type="number" class="solid" v-model="item['count']" />
				</view>
			</view>
