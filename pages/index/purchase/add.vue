<template>
	<view>
		<cu-custom v-if="showTop" bgColor="xg-nav" :isBack="true">
			...
			<block slot="content">新增采购单</block>
			<block slot="right">
				<view class="flex padding-sm margin-xs justify-end" @tap="showSavePurchase">
		...
		<form>
			<view class="cu-form-group" @click="showSearchChoose(2)">
				<view class="title">款号</view>
				<input disabled="true" placeholder="请选择" :value="order['style_code']"></input>
		...
		<view class="margin-top bg-white" v-for="(item,index) in mainPurchase" :key="index">
			<scroll-view scroll-x="true">
				...
				<view class="gg-scroll-view text-black">
					...
					<view class="gg-scroll-td td-b">
						<button class="cu-btn sm line-gblue margin-left-xs" @tap="editOne(index)">修改</button>
						<button class="cu-btn sm line-gpink margin-left-xs" @tap="delOne(index)">删除</button>
					</view>
							...
							<form>
								<view class="grid col-2">
									...
									<view class="cu-form-group" @click="showSinglePicker(99,index,ix)">
										...
										<input disabled="true" placeholder="请选择" v-model="thing['is_main_name']"></input>
										<text class="cuIcon-right"></text>
									</view>
									<view class="cu-form-group">
			...
			<view class="cu-dialog">
				...
				<scroll-view scroll-y="true" @scrolltolower="getMoreList" :style="[{top:CustomBar+'px',height:'calc(100vh - ' + CustomBar + 'px)'}]">
					...
					<view v-if="searchType==2" class="text-left margin-top-sm">
						<view v-for="(it,index) in designList" :key="index"
						 @tap="chooseDesign(it)">
							<view class="padding bg-white margin-top">
								<view class="flex">
									<view class="flex-sub cf padding-sm">
...
<script>
	...
	export default {
		...
		methods: {
			...
			//选择款号
			chooseDesign(it) {
				this.order.design_id = it.id
				this.order.style_code = it.style_code
				this.order.batch = 1
				this.order.size_ids = it.size_ids
				this.order.size_names = it.size_names
				this.order.color_ids = it.color_ids
				this.order.color_names = it.color_names
				this.hideModal(2)
			},
			//选择物料
			chooseMaterial(it) {
			...
			showSinglePicker(k,index,ix) {
				...
				this.pickerKey = k
				if (k == 99) {
					this.pickerValueArray = [{label:'是',value:1},{label:'否',value:2}]
				} else {
			...
			savePurchaseSubmit() {
				...
				api.post({
					url: 'home/purchases',
					data: {order:JSON.stringify(this.order), purchase:JSON.stringify(this.mainPurchase)},
					success: res => {
						console.log(res);
						uni.showToast({
							icon: 'none',
