<template>
	<view>
		<cu-custom bgColor="xg-nav" :isDrawer="true"><block slot="content">首页</block></cu-custom>
		
		<view>
			<view class="example">
				<view class="example-title" v-if="gridData[1]!=undefined">设计</view>
				<uni-grid :options="gridData[1]" @click="gridOne" :show-out-border="false" />
				<view class="example-title" v-if="gridData[2]!=undefined">采购</view>
				<uni-grid :options="gridData[2]" @click="gridTwo" :show-out-border="false" />
				<view class="example-title" v-if="gridData[3]!=undefined">仓库</view>
	...
	export default {
		...
		data() {
			return {
				...
				swiperGridWidth: '100%',
				gridData: uni.getStorageSync('menu')[1]
			}
		},
		onLoad() {
